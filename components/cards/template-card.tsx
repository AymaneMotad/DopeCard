...
export function TemplateCard({ template, onSelect, delay = 0 }: TemplateCardProps) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <Card className="relative h-full overflow-hidden border-0 shadow-lg group cursor-pointer">
        {/* Gradient Background */}
        <div
          className={cn(
        ...
        <CardContent className="relative z-10 p-6 flex flex-col h-full">
          ...
          <div className="flex-1">
            ...
            <div className="mb-4 p-3 rounded-lg bg-white/10 backdrop-blur-sm">
              <p className="text-xs text-white/80">{template.preview.description}</p>
            </div>
          ...
          <Button
            ...
            className="w-full mt-4 bg-white/20 hover:bg-white/30 text-white border-2 border-white/30 backdrop-blur-sm group/btn"
          >
